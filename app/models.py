...
class CycleBehavior(str, enum.Enum):
    ...
    # Next due date shifts on late payment (Netflix, gym)
    FLEXIBLE = "FLEXIBLE"
...
class RecurringEvent(Base):
    ...
    recurring_expense_id = Column(Integer, ForeignKey(
        "recurring_expenses.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    recurring_expense = relationship(
        "RecurringExpense", back_populates="events")
...
class UserProfile(Base):
    ...
    # Old single status, keeping for migration
    life_status = Column(Enum(LifeStatus), nullable=True)
    life_statuses = Column(JSON, nullable=False,
                           server_default='[]')  # New multi-status
...
class Wallet(Base):
    ...
    accounting_type = Column(Enum(AccountingType),
                             nullable=False, default=AccountingType.ASSET)
    ...
    can_fund_goals = Column(Boolean, default=False,
    ...
    ledger_entries = relationship(
        "WalletLedger", back_populates="wallet", cascade="all, delete-orphan")
    goal_contributions = relationship(
        "GoalContributions", back_populates="wallet")
    isolated_project_allocations = relationship(
        "IsolatedProjectWalletAllocation", back_populates="wallet")
...
class WalletTransfer(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_wallet_transfers_amount_positive"),
        CheckConstraint("amount <= 999999999999",
                        name="ck_wallet_transfers_amount_limit"),
        CheckConstraint("from_wallet_id != to_wallet_id",
    ...
    owner_id = Column(Integer, ForeignKey(
        "users.id", ondelete="CASCADE"), nullable=False, index=True)
    from_wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="CASCADE"), nullable=False)
    to_wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="CASCADE"), nullable=False)
...
class IncomeSource(Base):
    ...
    expected_inflow_promises = relationship(
        "ExpectedInflowPromise", back_populates="source")
...
class IncomeEntry(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("amount <= 999999999999",
                        name="ck_income_entries_amount_limit"),
...
class ExpectedInflowPromise(Base):
    ...
    __table_args__ = (
        CheckConstraint("original_amount > 0",
                        name="ck_expected_inflow_promises_amount_positive"),
        CheckConstraint("original_amount <= 999999999999",
                        name="ck_expected_inflow_promises_amount_limit"),
        CheckConstraint(
    ...
    kind = Column(String(32), nullable=False,
                  default=ExpectedInflowKind.EARNED.value, index=True)
    source_id = Column(Integer, ForeignKey(
        "income_sources.id", ondelete="RESTRICT"), nullable=True, index=True)
    debt_id = Column(Integer, ForeignKey(
        "debts.id", ondelete="RESTRICT"), nullable=True, index=True)
    asset_id = Column(Integer, ForeignKey(
        "assets.id", ondelete="RESTRICT"), nullable=True, index=True)
    refund_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="RESTRICT"), nullable=True, index=True)
    ...
    source = relationship(
        "IncomeSource", back_populates="expected_inflow_promises")
    debt = relationship("Debt", foreign_keys=[debt_id])
    asset = relationship("Asset", foreign_keys=[asset_id])
    refund_event = relationship(
        "FinancialEvent", foreign_keys=[refund_event_id])
    ...
    realizations = relationship(
        "ExpectedInflowRealization", back_populates="promise")
...
class ExpectedIncome(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_expected_incomes_amount_positive"),
        CheckConstraint("amount <= 999999999999",
                        name="ck_expected_incomes_amount_limit"),
        CheckConstraint("due_date >= '2020-01-01'",
                        name="ck_expected_incomes_due_date_min_2020_01_01"),
        CheckConstraint("budget_month >= 1 AND budget_month <= 12",
                        name="ck_expected_incomes_budget_month_1_12"),
        CheckConstraint("budget_year >= 2020",
                        name="ck_expected_incomes_budget_year_min_2020"),
        CheckConstraint(
            "(CASE WHEN source_id IS NOT NULL THEN 1 ELSE 0 END + "
            "CASE WHEN debt_id IS NOT NULL THEN 1 ELSE 0 END + "
            "CASE WHEN asset_id IS NOT NULL THEN 1 ELSE 0 END + "
        ...
        Index("ix_expected_incomes_owner_month_status",
              "owner_id", "budget_year", "budget_month", "status"),
        Index("ix_expected_incomes_owner_kind_status",
    ...
    source_id = Column(Integer, ForeignKey(
        "income_sources.id", ondelete="SET NULL"), nullable=True, index=True)
    debt_id = Column(Integer, ForeignKey(
        "debts.id", ondelete="SET NULL"), nullable=True, index=True)
    asset_id = Column(Integer, ForeignKey(
        "assets.id", ondelete="SET NULL"), nullable=True, index=True)
    refund_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    parent_id = Column(Integer, ForeignKey(
        "expected_incomes.id", ondelete="RESTRICT"), nullable=True, index=True)
    amount = Column(BigInteger, nullable=False)
    received_amount = Column(BigInteger, nullable=True, default=0)
    linked_transaction_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    due_date = Column(Date, nullable=False)
    budget_year = Column(Integer, nullable=False)
    budget_month = Column(Integer, nullable=False)
    status = Column(Enum(ExpectedIncomeStatus), nullable=False,
                    default=ExpectedIncomeStatus.EXPECTED)
    backing_eligible = Column(Boolean, nullable=False, default=True)
    ...
    linked_transaction = relationship(
        "FinancialEvent", foreign_keys=[linked_transaction_id])
    parent = relationship("ExpectedIncome", remote_side=[
                          id], back_populates="children", foreign_keys=[parent_id])
    children = relationship("ExpectedIncome", back_populates="parent", foreign_keys=[
                            parent_id], passive_deletes=True)
...
class ExpectedInflowRealization(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_expected_inflow_realizations_owner_date",
...
class ExpectedInflowRealizationAllocation(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_expected_inflow_realization_allocations_amount_positive"),
        UniqueConstraint(
            "realization_id",
            "expected_inflow_id",
    ...
    realization = relationship(
        "ExpectedInflowRealization", back_populates="allocations")
    expected_inflow = relationship(
        "ExpectedIncome", back_populates="realization_allocations")
...
class ExpectedInflowRealizationEvent(Base):
    ...
    realization = relationship(
        "ExpectedInflowRealization", back_populates="event_links")
...
class ExpectedInflowWriteOff(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_expected_inflow_write_offs_amount_positive"),
        CheckConstraint("amount <= 999999999999",
                        name="ck_expected_inflow_write_offs_amount_limit"),
        Index("ix_expected_inflow_write_offs_promise_date",
...
class RecurringExpense(Base):
    ...
    # ── Cycle Behavior ────────────────────────────────────────────────
    cycle_behavior = Column(Enum(CycleBehavior),
                            nullable=False, default=CycleBehavior.FIXED)
    ...
    # Relationships
    events = relationship(
        "RecurringEvent", back_populates="recurring_expense", cascade="all, delete-orphan")
...
class RecurringOccurrence(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_recurring_occurrences_owner_due_date",
...
class Budget(Base):
    ...
    entity_ledger_entries = relationship(
        "EntityLedger", back_populates="budget")
    ...
    __table_args__ = (
        ...
        CheckConstraint("monthly_limit <= 999999999999",
                        name="ck_budgets_monthly_limit"),
...
class BorrowingSurvivalPlan(Base):
    ...
    monthly_cap = Column(BigInteger, nullable=False,
...
class Goals(Base):
    ...
    currency = Column(String(3), default="UZS",
                      server_default="UZS", nullable=False)
    intent = Column(
        Enum(GoalIntent),
        nullable=False,
    ...
    linked_asset_id = Column(Integer, ForeignKey(
        "assets.id", ondelete="SET NULL"), nullable=True, index=True)
    linked_debt_id = Column(Integer, ForeignKey(
        "debts.id", ondelete="SET NULL"), nullable=True, index=True)
    linked_debt_transaction_id = Column(Integer, ForeignKey(
        "debt_transactions.id", ondelete="SET NULL"), nullable=True, index=True)
    linked_payment_plan_id = Column(Integer, ForeignKey(
        "payment_plans.id", ondelete="SET NULL"), nullable=True, index=True)
    linked_expense_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    created_at = Column(DateTime(timezone=True),
                        server_default=func.now(), nullable=False)
    ...
    linked_debt_transaction = relationship("DebtTransaction", foreign_keys=[
                                           linked_debt_transaction_id])
    linked_payment_plan = relationship(
        "PaymentPlan", foreign_keys=[linked_payment_plan_id])
    linked_expense_event = relationship(
        "FinancialEvent", foreign_keys=[linked_expense_event_id])
...
class GoalContributions(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_goal_contributions_wallet_created_at",
    ...
    wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="RESTRICT"), nullable=False, index=True)
    linked_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    amount = Column(BigInteger, nullable=False)
...
class GoalProjectRelease(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_goal_project_releases_amount_positive"),
        Index("ix_goal_project_releases_goal_created_at",
              "goal_id", "created_at"),
        Index("ix_goal_project_releases_project_created_at",
    ...
    wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="RESTRICT"), nullable=True, index=True)
    amount = Column(BigInteger, nullable=False)
...
class Debt(Base):
    ...
    __table_args__ = (
        CheckConstraint("initial_amount > 0",
                        name="ck_debts_initial_amount_positive"),
        CheckConstraint("remaining_amount >= 0",
                        name="ck_debts_remaining_amount_non_negative"),
        CheckConstraint("date >= '2020-01-01'",
                        name="ck_debts_date_min_2020_01_01"),
        CheckConstraint("expected_return_date >= '2020-01-01'",
    ...
    transactions = relationship(
        "DebtTransaction", back_populates="debt", cascade="all, delete-orphan")
    charges = relationship(
        "DebtCharge", back_populates="debt", cascade="all, delete-orphan")
    ledger_entries = relationship(
        "DebtLedgerEntry", back_populates="debt", cascade="all, delete-orphan")
    formal_details = relationship(
        "DebtFormalDetails", back_populates="debt", uselist=False, cascade="all, delete-orphan")
    transaction_wallet_allocations = relationship(
        "DebtTransactionWalletAllocation", back_populates="debt", cascade="all, delete-orphan")
    asset_settlements = relationship(
        "DebtAssetSettlement", back_populates="debt", cascade="all, delete-orphan")
    payment_plan = relationship(
        "PaymentPlan", back_populates="debt", uselist=False)
    ...
    initial_wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="SET NULL"), nullable=True)
    linked_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True)

    # Pre-tags for Cash-Basis Accounting
    expense_category = Column(Enum(ExpenseCategory), nullable=True)
    expense_subcategory_id = Column(Integer, ForeignKey(
        "user_subcategories.id", ondelete="SET NULL"), nullable=True)
    project_id = Column(Integer, ForeignKey("projects.id", ondelete="SET NULL",
                        use_alter=True, name="fk_debts_project_id"), nullable=True)
    project_subcategory_id = Column(Integer, ForeignKey("legacy_project_subcategories.id",
                                    ondelete="SET NULL", use_alter=True, name="fk_debts_project_subcategory_id"), nullable=True)
    income_source_id = Column(Integer, ForeignKey(
        "income_sources.id", ondelete="SET NULL"), nullable=True)
...
class DebtTransaction(Base):
    ...
    wallet = relationship("Wallet")
    debt_id = Column(Integer, ForeignKey(
        "debts.id", ondelete="CASCADE"), nullable=False, index=True)
    amount = Column(BigInteger, nullable=False)
    date = Column(Date, nullable=False)
    note = Column(String(200), nullable=True)
    created_at = Column(DateTime(timezone=True),
    ...
    wallet_allocations = relationship(
        "DebtTransactionWalletAllocation", back_populates="transaction", cascade="all, delete-orphan")
...
class DebtTransactionWalletAllocation(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_debt_transaction_wallet_allocations_amount_positive"),
        UniqueConstraint("debt_transaction_id", "wallet_id",
                         name="uq_debt_transaction_wallet_allocations_wallet"),
    )
    ...
    debt_id = Column(Integer, ForeignKey(
        "debts.id", ondelete="CASCADE"), nullable=False, index=True)
    debt_transaction_id = Column(Integer, ForeignKey(
        "debt_transactions.id", ondelete="CASCADE"), nullable=False, index=True)
    wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="RESTRICT"), nullable=False, index=True)
    amount = Column(BigInteger, nullable=False)
    created_at = Column(DateTime(timezone=True),
    ...
    owner = relationship(
        "User", back_populates="debt_transaction_wallet_allocations")
    debt = relationship(
        "Debt", back_populates="transaction_wallet_allocations")
    transaction = relationship(
        "DebtTransaction", back_populates="wallet_allocations")
    wallet = relationship("Wallet")
...
class DebtCharge(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("amount <= 999999999999",
                        name="ck_debt_charges_amount_limit"),
    ...
    owner_id = Column(Integer, ForeignKey(
        "users.id", ondelete="CASCADE"), nullable=False, index=True)
    debt_id = Column(Integer, ForeignKey(
        "debts.id", ondelete="CASCADE"), nullable=False)
    amount = Column(BigInteger, nullable=False)
...
class DebtLedgerEntry(Base):
    ...
    __table_args__ = (
        CheckConstraint("amount_delta != 0",
                        name="ck_debt_ledger_amount_delta_non_zero"),
        ...
        Index("ix_debt_ledger_source_transaction_id",
    ...
    debt_id = Column(Integer, ForeignKey(
        "debts.id", ondelete="CASCADE"), nullable=False, index=True)
    financial_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    source_debt_transaction_id = Column(Integer, ForeignKey(
        "debt_transactions.id", ondelete="SET NULL"), nullable=True, index=True)
    source_debt_charge_id = Column(Integer, ForeignKey(
        "debt_charges.id", ondelete="SET NULL"), nullable=True, index=True)
    reverses_entry_id = Column(Integer, ForeignKey(
        "debt_ledger_entries.id", ondelete="SET NULL"), nullable=True, index=True)
    wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="SET NULL"), nullable=True, index=True)
    asset_id = Column(Integer, ForeignKey(
        "assets.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    principal_delta = Column(BigInteger, nullable=False,
                             default=0, server_default="0")
    charge_delta = Column(BigInteger, nullable=False,
                          default=0, server_default="0")
    ...
    status = Column(String(20), nullable=False,
                    default="POSTED", server_default="POSTED")
...
class DebtFormalDetails(Base):
    ...
    __table_args__ = (
        CheckConstraint("statement_balance IS NULL OR statement_balance >= 0",
                        name="ck_debt_formal_statement_balance_non_negative"),
        CheckConstraint("annual_rate_bps IS NULL OR annual_rate_bps >= 0",
                        name="ck_debt_formal_annual_rate_bps_non_negative"),
    )

    debt_id = Column(Integer, ForeignKey(
        "debts.id", ondelete="CASCADE"), primary_key=True)
    owner_id = Column(Integer, ForeignKey(
        "users.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    linked_asset_id = Column(Integer, ForeignKey(
        "assets.id", ondelete="SET NULL"), nullable=True, index=True)
    collateral_asset_id = Column(Integer, ForeignKey(
        "assets.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    linked_asset = relationship("Asset", foreign_keys=[linked_asset_id])
    collateral_asset = relationship(
        "Asset", foreign_keys=[collateral_asset_id])
...
class DebtAssetSettlement(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_debt_asset_settlements_amount_positive"),
        Index("ix_debt_asset_settlements_debt_date",
              "debt_id", "settlement_date"),
    )
    ...
    financial_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    debt_ledger_entry_id = Column(Integer, ForeignKey(
        "debt_ledger_entries.id", ondelete="SET NULL"), nullable=True, index=True)
...
class PaymentPlanTransaction(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_payment_plan_transactions_amount_positive"),
    )
    ...
    wallet_allocations = relationship(
        "PaymentPlanTransactionWalletAllocation", back_populates="transaction", cascade="all, delete-orphan")
...
class PaymentPlanTransactionWalletAllocation(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_payment_plan_txn_wallet_allocations_amount_positive"),
        UniqueConstraint("payment_plan_transaction_id", "wallet_id",
                         name="uq_payment_plan_txn_wallet_allocations_wallet"),
    )
    ...
    plan_id = Column(Integer, ForeignKey("payment_plans.id",
                     ondelete="CASCADE"), nullable=False, index=True)
    payment_plan_transaction_id = Column(Integer, ForeignKey(
        "payment_plan_transactions.id", ondelete="CASCADE"), nullable=False, index=True)
    wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="RESTRICT"), nullable=False, index=True)
    amount = Column(BigInteger, nullable=False)
    created_at = Column(DateTime(timezone=True),
    ...
    owner = relationship(
        "User", back_populates="payment_plan_transaction_wallet_allocations")
    plan = relationship(
        "PaymentPlan", back_populates="transaction_wallet_allocations")
    transaction = relationship(
        "PaymentPlanTransaction", back_populates="wallet_allocations")
    wallet = relationship("Wallet")
...
class PaymentPlanCharge(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_payment_plan_charges_amount_positive"),
    ...
    plan_id = Column(Integer, ForeignKey(
        "payment_plans.id", ondelete="CASCADE"), nullable=False)
    amount = Column(BigInteger, nullable=False)
    reason = Column(String(200), nullable=True)
    date = Column(Date, nullable=False)
    created_at = Column(DateTime(timezone=True),
...
class PaymentPlanLedgerEntry(Base):
    ...
    __table_args__ = (
        CheckConstraint("amount_delta != 0",
                        name="ck_payment_plan_ledger_amount_delta_non_zero"),
    ...
    financial_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    source_transaction_id = Column(Integer, ForeignKey(
        "payment_plan_transactions.id", ondelete="SET NULL"), nullable=True, index=True)
    source_charge_id = Column(Integer, ForeignKey(
        "payment_plan_charges.id", ondelete="SET NULL"), nullable=True, index=True)
    reverses_entry_id = Column(Integer, ForeignKey(
...
class PaymentPlan(Base):
    ...
    __table_args__ = (
        CheckConstraint("total_price > 0",
                        name="ck_payment_plans_total_price_positive"),
        CheckConstraint("down_payment >= 0",
                        name="ck_payment_plans_down_payment_non_negative"),
        CheckConstraint("remaining_amount >= 0",
                        name="ck_payment_plans_remaining_amount_non_negative"),
        CheckConstraint("months > 0", name="ck_payment_plans_months_positive"),
        CheckConstraint("payment_count > 0",
                        name="ck_payment_plans_payment_count_positive"),
        CheckConstraint("regular_payment_amount >= 0",
                        name="ck_payment_plans_regular_payment_amount_non_negative"),
        CheckConstraint("start_date >= '2020-01-01'",
                        name="ck_payment_plans_start_date_min_2020_01_01"),
    ...
    owner_id = Column(Integer, ForeignKey(
        "users.id", ondelete="CASCADE"), nullable=False, index=True)
    debt_id = Column(Integer, ForeignKey("debts.id", ondelete="SET NULL",
                     use_alter=True, name="fk_payment_plans_debt_id"), nullable=True)
    ...
    frequency = Column(Enum(PaymentPlanFrequency),
                       nullable=False, default=PaymentPlanFrequency.MONTHLY)
    ...
    status = Column(Enum(PaymentPlanStatus), nullable=False,
    ...
    expense_subcategory_id = Column(Integer, ForeignKey(
        "user_subcategories.id", ondelete="SET NULL"), nullable=True)
    project_id = Column(Integer, ForeignKey("projects.id", ondelete="SET NULL",
                        use_alter=True, name="fk_payment_plans_project_id"), nullable=True)
    project_subcategory_id = Column(Integer, ForeignKey("legacy_project_subcategories.id", ondelete="SET NULL",
    ...
    payments = relationship("PaymentPlanPayment",
                            back_populates="plan", cascade="all, delete")
    transactions = relationship(
        "PaymentPlanTransaction", back_populates="plan", cascade="all, delete")
    charges = relationship("PaymentPlanCharge",
                           back_populates="plan", cascade="all, delete")
    ledger_entries = relationship(
        "PaymentPlanLedgerEntry", back_populates="plan", cascade="all, delete")
    transaction_wallet_allocations = relationship(
        "PaymentPlanTransactionWalletAllocation", back_populates="plan", cascade="all, delete-orphan")
...
class PaymentPlanPayment(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_payment_plan_payments_amount_positive"),
        CheckConstraint(
            "paid_amount >= 0", name="ck_payment_plan_payments_paid_amount_non_negative"),
        CheckConstraint("written_off_amount >= 0",
                        name="ck_payment_plan_payments_written_off_amount_non_negative"),
        CheckConstraint("paid_amount <= amount",
                        name="ck_payment_plan_payments_paid_amount_not_above_amount"),
        CheckConstraint(
            "paid_amount + written_off_amount <= amount",
            name="ck_payment_plan_payments_settled_amount_not_above_amount",
        ),
        CheckConstraint("due_date >= '2020-01-01'",
                        name="ck_payment_plan_payments_due_date_min_2020_01_01"),
    ...
    plan_id = Column(Integer, ForeignKey("payment_plans.id",
                     ondelete="CASCADE"), nullable=False, index=True)
    payment_plan_charge_id = Column(Integer, ForeignKey(
        "payment_plan_charges.id", ondelete="SET NULL"), nullable=True, index=True)
    amount = Column(BigInteger, nullable=False)
    paid_amount = Column(BigInteger, nullable=False,
                         default=0, server_default="0")
    written_off_amount = Column(
    ...
    status = Column(Enum(PaymentPlanPaymentStatus), nullable=False,
                    default=PaymentPlanPaymentStatus.PENDING)
    due_date = Column(Date, nullable=False)
    ...
    allocations = relationship("PaymentPlanPaymentAllocation",
...
class PaymentPlanPaymentAllocation(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_payment_plan_payment_allocations_amount_positive"),
        Index("ix_payment_plan_payment_allocations_owner_date",
              "owner_id", "paid_date"),
    )
    ...
    owner_id = Column(Integer, ForeignKey(
        "users.id", ondelete="CASCADE"), nullable=False, index=True)
    payment_plan_payment_id = Column(Integer, ForeignKey(
        "payment_plan_payments.id", ondelete="CASCADE"), nullable=False, index=True)
    financial_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    payment_plan_transaction_id = Column(Integer, ForeignKey(
        "payment_plan_transactions.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    owner = relationship(
        "User", back_populates="payment_plan_payment_allocations")
...
class UserSubcategory(Base):
    ...
    __table_args__ = (
        UniqueConstraint("owner_id", "category", "name",
                         name="uq_user_subcats_owner_cat_name"),
...
class BudgetSubcategoryLimit(Base):
    ...
    __table_args__ = (
        UniqueConstraint("budget_id", "subcategory_id",
                         name="uq_budget_subcategory_limits_budget_subcategory"),
        CheckConstraint(
            "monthly_limit > 0", name="ck_budget_subcategory_limits_monthly_limit_positive"),
        CheckConstraint("monthly_limit <= 999999999999",
                        name="ck_budget_subcategory_limits_monthly_limit_max"),
    ...
    subcategory_id = Column(Integer, ForeignKey(
        "user_subcategories.id", ondelete="CASCADE"), nullable=False)
    monthly_limit = Column(BigInteger, nullable=False)
    created_at = Column(DateTime(timezone=True),
                        server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(
    ...
    subcategory = relationship(
        "UserSubcategory", back_populates="monthly_limits")
...
class BudgetLedger(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_budget_ledger_lookup", "owner_id",
              "category", "budget_year", "budget_month")
    )
...
class Project(Base):
    ...
    project_type = Column(Enum(ProjectType), nullable=False,
                          default=ProjectType.OVERLAY)

    # Optional Graduation pipeline
    origin_goal_id = Column(Integer, ForeignKey(
        "goals.id", ondelete="SET NULL"), nullable=True)
    ...
    legacy_subcategories = relationship(
        "LegacyProjectSubcategory", back_populates="project", cascade="all, delete-orphan")
    ...
    goal_releases = relationship(
        "GoalProjectRelease", back_populates="project", cascade="all, delete-orphan")
    isolated_wallet_allocations = relationship(
        "IsolatedProjectWalletAllocation",
        back_populates="project",
        cascade="all, delete-orphan",
    )
    session_draft_items = relationship(
        "ExpenseSessionDraftItem", back_populates="project")
...
class ProjectOverlayDetail(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "project_id", name="uq_project_overlay_details_project_id"),
        CheckConstraint("target_estimate IS NULL OR target_estimate > 0",
...
class ProjectIsolatedDetail(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "project_id", name="uq_project_isolated_details_project_id"),
        CheckConstraint("funding_limit IS NULL OR funding_limit > 0",
...
class IsolatedProjectWalletAllocation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("project_id", "wallet_id",
                         name="uq_isolated_project_wallet_allocations_project_wallet"),
        CheckConstraint(
            "amount > 0", name="ck_isolated_project_wallet_allocations_amount_positive"),
        CheckConstraint("amount <= 999999999999",
                        name="ck_isolated_project_wallet_allocations_amount_max"),
        Index("ix_isolated_project_wallet_allocations_owner_wallet",
              "owner_id", "wallet_id"),
    )
    ...
    project = relationship(
        "Project", back_populates="isolated_wallet_allocations")
    owner = relationship(
        "User", back_populates="isolated_project_wallet_allocations")
    wallet = relationship(
        "Wallet", back_populates="isolated_project_allocations")
...
class IsolatedProjectCategoryAllocation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("project_id", "category",
                         name="uq_isolated_project_category_allocations_project_category"),
    )
    ...
    project = relationship(
        "Project", back_populates="isolated_category_allocations")
...
class OverlayProjectCategoryReservation(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("budget_month >= 1 AND budget_month <= 12",
                        name="ck_overlay_project_category_reservations_month"),
        CheckConstraint("budget_year >= 2020",
                        name="ck_overlay_project_category_reservations_year"),
        CheckConstraint(
            "limit_amount > 0", name="ck_overlay_project_category_reservations_amount_positive"),
        CheckConstraint("limit_amount <= 999999999999",
                        name="ck_overlay_project_category_reservations_amount_max"),
        Index("ix_overlay_project_category_reservations_month",
    ...
    project = relationship(
        "Project", back_populates="overlay_category_reservations")
...
class LegacyProjectSubcategory(Base):
    ...
    __table_args__ = (
        UniqueConstraint("project_id", "category", "name",
                         name="uq_legacy_project_subcategories_project_category_name"),
        Index("ix_legacy_project_subcategories_project_category",
              "project_id", "category"),
    )
...
class IsolatedProjectSubcategoryAllocation(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("allocated_amount > 0",
                        name="ck_isolated_project_subcategory_allocations_amount_positive"),
        CheckConstraint("allocated_amount <= 999999999999",
                        name="ck_isolated_project_subcategory_allocations_amount_max"),
        Index("ix_isolated_project_subcategory_allocations_project_category",
              "project_id", "category"),
        Index("ix_isolated_project_subcategory_allocations_taxonomy",
              "user_subcategory_id"),
    )
    ...
    category_allocation_id = Column(
        Integer,
        ForeignKey("isolated_project_category_allocations.id",
                   ondelete="CASCADE"),
        nullable=True,
        index=True,
    )
    category = Column(Enum(ExpenseCategory), nullable=False)
    user_subcategory_id = Column(Integer, ForeignKey(
        "user_subcategories.id", ondelete="RESTRICT"), nullable=False)
...
class OverlayProjectSubcategoryReservation(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("budget_month >= 1 AND budget_month <= 12",
                        name="ck_overlay_project_subcategory_reservations_month"),
        CheckConstraint("budget_year >= 2020",
                        name="ck_overlay_project_subcategory_reservations_year"),
        CheckConstraint(
            "limit_amount > 0", name="ck_overlay_project_subcategory_reservations_amount_positive"),
        CheckConstraint("limit_amount <= 999999999999",
                        name="ck_overlay_project_subcategory_reservations_amount_max"),
        Index("ix_overlay_project_subcategory_reservations_month",
              "budget_year", "budget_month", "category"),
        Index("ix_overlay_project_subcategory_reservations_subcategory",
              "user_subcategory_id"),
    )
    ...
    project = relationship(
        "Project", back_populates="overlay_subcategory_reservations")
    user_subcategory = relationship(
        "UserSubcategory", back_populates="project_monthly_limits")


class ExpenseSessionDraft(Base):
    __tablename__ = "expense_session_drafts"
    __table_args__ = (
        CheckConstraint("amount_paid IS NULL OR amount_paid > 0",
                        name="ck_expense_session_drafts_amount_paid_positive"),
    ...
    status = Column(Enum(ExpenseSessionDraftStatus),
                    nullable=False, default=ExpenseSessionDraftStatus.ACTIVE)
    source_type = Column(Enum(ExpenseSessionDraftSource),
                         nullable=False, default=ExpenseSessionDraftSource.MANUAL)
    raw_ocr_text = Column(String, nullable=True)
    finalized_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    created_at = Column(DateTime(timezone=True),
                        server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(
    ...
    finalized_event = relationship(
        "FinancialEvent", foreign_keys=[finalized_event_id])
    items = relationship("ExpenseSessionDraftItem",
                         back_populates="draft", cascade="all, delete-orphan")
    wallet_allocations = relationship(
        "ExpenseSessionDraftWalletAllocation", back_populates="draft", cascade="all, delete-orphan"
    )
    splits = relationship("ExpenseSessionDraftSplit",
                          back_populates="draft", cascade="all, delete-orphan")
...
class ExpenseSessionDraftItem(Base):
    ...
    __table_args__ = (
        CheckConstraint("original_amount > 0",
                        name="ck_expense_session_draft_items_original_amount_positive"),
    )

    id = Column(Integer, primary_key=True, index=True)
    draft_id = Column(Integer, ForeignKey(
        "expense_session_drafts.id", ondelete="CASCADE"), nullable=False, index=True)
    owner_id = Column(Integer, ForeignKey(
        "users.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    project_subcategory_id = Column(Integer, ForeignKey(
        "legacy_project_subcategories.id", ondelete="SET NULL"), nullable=True, index=True)
...
class ExpenseSessionDraftWalletAllocation(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_expense_session_draft_wallet_allocations_amount_positive"),
        UniqueConstraint("draft_id", "wallet_id",
                         name="uq_expense_session_draft_wallet_allocations_draft_wallet"),
    )
    ...
    owner_id = Column(Integer, ForeignKey(
        "users.id", ondelete="CASCADE"), nullable=False, index=True)
    wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="CASCADE"), nullable=False, index=True)
    amount = Column(BigInteger, nullable=False)
    created_at = Column(DateTime(timezone=True),
                        server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(
    ...
    draft = relationship("ExpenseSessionDraft",
                         back_populates="wallet_allocations")
    owner = relationship(
        "User", back_populates="expense_session_draft_wallet_allocations")
    wallet = relationship("Wallet")
...
class ExpenseSessionDraftSplit(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0", name="ck_expense_session_draft_splits_amount_positive"),
    )
...
class FinancialEvent(Base):
    ...
    status = Column(Enum(FinancialEventStatus), nullable=False, default=FinancialEventStatus.POSTED,
                    server_default=FinancialEventStatus.POSTED.value, index=True)
    ...
    linked_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True)
    merge_group_id = Column(Integer, ForeignKey(
        "expense_merge_groups.id", ondelete="SET NULL"), nullable=True, index=True)
    void_reversal_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    reverses_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    wallet_legs = relationship(
        "WalletLedger", back_populates="event", cascade="all, delete-orphan")
    entity_legs = relationship(
        "EntityLedger", back_populates="event", cascade="all, delete-orphan")
...
class WalletLedger(Base):
    ...
    owner_id = Column(Integer, ForeignKey(
        "users.id", ondelete="CASCADE"), nullable=False, index=True)
    event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="CASCADE"), nullable=False, index=True)
    wallet_id = Column(Integer, ForeignKey(
        "wallets.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class EntityLedger(Base):
    ...
    # Item title within a session ("Beef", "Eggs")
    label = Column(String(100), nullable=True)
    # Positive = Budget Space Gained, Negative = Space Drained
    amount = Column(BigInteger, nullable=False)
    # Pre-discount price (NULL if no discount)
    original_amount = Column(BigInteger, nullable=True)
    ...
    project_subcategory_id = Column(Integer, ForeignKey(
        "legacy_project_subcategories.id", ondelete="SET NULL"), nullable=True, index=True)
    budget_id = Column(Integer, ForeignKey(
        "budgets.id", ondelete="SET NULL"), nullable=True)
    debt_id = Column(Integer, ForeignKey(
        "debts.id", ondelete="SET NULL"), nullable=True)
    income_source_id = Column(Integer, ForeignKey(
        "income_sources.id", ondelete="SET NULL"), nullable=True)
    payment_plan_id = Column(Integer, ForeignKey(
        "payment_plans.id", ondelete="SET NULL"), nullable=True, index=True)
    payment_plan_payment_id = Column(Integer, ForeignKey(
        "payment_plan_payments.id", ondelete="SET NULL"), nullable=True, index=True)
...
class Asset(Base):
    ...
    __table_args__ = (
        CheckConstraint("purchase_value > 0",
                        name="ck_assets_purchase_value_positive"),
        CheckConstraint("current_value >= 0",
                        name="ck_assets_current_value_non_negative"),
    ...
    origin_event_id = Column(Integer, ForeignKey(
        "financial_events.id", ondelete="SET NULL"), nullable=True)
