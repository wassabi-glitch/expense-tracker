...
from ..services.overlay_project_service import (
    get_owned_project_subcategory_reservation_or_404,
...
def create_project(
    ...
    if project_type == models.ProjectType.ISOLATED:
        ...
        for item in category_allocations:
            ...
            db.add(alloc)
            category_allocs_map[item.category] = alloc
        db.flush()

        if payload.subcategory_allocations:
            subcat_sums = {}
            for sub_payload in payload.subcategory_allocations:
                ...
                if not user_subcat:
                    user_subcat = models.UserSubcategory(
                        owner_id=current_user.id,
                        category=sub_payload.category,
                        name=name_clean,
                        is_active=True,
                    )
                    db.add(user_subcat)
                ...
                db.add(models.IsolatedProjectSubcategoryAllocation(
                    ...
                    allocated_amount=int(sub_payload.limit_amount),
                    is_active=sub_payload.is_active,
                ))
    else:
        db.add(
...
def create_overlay_project(
    ...
    for item in payload.category_reservations:
        ...
        db.add(
            models.OverlayProjectCategoryReservation(
                project_id=project.id,
                category=item.category,
                budget_year=payload.budget_year,
                budget_month=payload.budget_month,
...
def update_project(
    ...
    if (
        ...
        and project.isolated_wallet_allocations
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def list_project_category_limits(
    ...
    if is_isolated_project(project):
        ...
        for item in sorted(project.isolated_category_allocations, key=lambda value: str(value.category)):
            spent = get_isolated_project_category_spent_amount(
                db,
                current_user.id,
                project.id,
            ...
            rows.append(
                schemas.ProjectBudgetCategoryOut(
                    ...
                    allocated_amount=int(item.limit_amount),
                    spent=spent,
                    remaining=remaining,
    ...
    return [
        schemas.ProjectBudgetCategoryOut(
            category=item.category,
            limit_amount=int(item.limit_amount),
            reserved_amount=int(item.limit_amount),
...
def list_project_subcategories(
    ...
    if is_isolated_project(project):
        ...
        spent_by_subcategory = {
            (int(row.project_id), int(row.subcategory_id)): int(row.spent or 0)
        ...
        for item in rows:
            ...
            output.append(
                schemas.ProjectSubcategoryOut(
                    ...
                    user_subcategory_id=item.user_subcategory_id,
                    limit_amount=int(item.allocated_amount),
...
def create_project_subcategory(
    ...
    if is_isolated_project(project):
        ...
        parent_alloc = (
            db.query(models.IsolatedProjectCategoryAllocation)
            .filter(
                models.IsolatedProjectCategoryAllocation.project_id == project.id,
                models.IsolatedProjectCategoryAllocation.category == payload.category
            )
            .first()
        )
        if not parent_alloc:
        ...
        # Check existing active micro-subcategory for this taxonomy
        existing = (
            db.query(models.IsolatedProjectSubcategoryAllocation)
            .filter(
...
def update_project_subcategory(
    ...
    if is_isolated_project(project):
        ...
        if "name" in payload.model_fields_set and payload.name is not None:
            name_clean = payload.name.strip()
            if name_clean.lower() != allocation.user_subcategory.name.lower():
                user_subcat = (
                    db.query(models.UserSubcategory)
                    .filter(
                        models.UserSubcategory.owner_id == current_user.id,
                ...
                if not user_subcat:
                    ...
                    db.add(user_subcat)
                    db.flush()
                allocation.user_subcategory_id = user_subcat.id
        ...
        reservation = get_owned_project_subcategory_reservation_or_404(
            db,
            current_user.id,
            project_id,
            subcategory_id,
...
def delete_project_subcategory(
    ...
    if is_isolated_project(project):
        ...
        subcategory = get_owned_project_subcategory_reservation_or_404(
            db,
            current_user.id,
            project_id,
            subcategory_id,
