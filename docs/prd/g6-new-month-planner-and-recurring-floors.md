...
- Add a narrow month setup seam for explicit setup modes. All modes obey the **"Strict Limit Balancing"** rule (canceling previous "Permissive UX"). The system MUST intercept the user if a proposed plan exceeds their available capacity. The user is required to balance the budget (reduce limits or log an Expected Inflow) before proceeding.
...
- **Mode 2: Copy Previous Month:** Explicitly copies limits from the previous month. If these copied limits exceed current available funds, the UI presents an intercept screen forcing the user to trim limits until balanced.
- **Mode 3: Smart Auto-Fill (Smart Copy):** Copies the previous month but performs Category Floor repair. It aggregates standard Recurring Expenses AND Overlay Project Slices into the Category Floor calculation, automatically bumping limits up to at least the computed floor amount. If this total exceeds available funds, the same intercept rule applies.
