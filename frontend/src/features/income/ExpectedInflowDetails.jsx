...
export default function ExpectedInflowDetails() {
  ...
  return (
    <div className="w-full space-y-7 px-page py-8">
      ...
      <section className="space-y-3"><div className="flex items-center gap-2"><CalendarClock className="h-5 w-5" /><h2 className="text-lg font-semibold">Current schedule</h2></div>{activeSchedules.length === 0 ? <p className="text-sm text-muted-foreground">No active schedules.</p> : <div className="grid gap-3 lg:grid-cols-2">{activeSchedules.map((schedule) => <Card key={schedule.id} className="rounded-lg"><CardContent className="space-y-2 p-4"><div className="flex items-center justify-between gap-3"><p className="font-medium">{formatDisplayDate(schedule.due_date)}</p><Badge variant="outline">{READ_STATE_LABELS[schedule.read_state] || schedule.read_state}</Badge></div><div className="grid grid-cols-2 gap-2 text-sm text-muted-foreground"><span>Scheduled {schedule.amount}</span><span>Remaining {schedule.remaining_amount}</span><span>Received {schedule.received_amount}</span><span>Written off {schedule.written_off_amount}</span></div></CardContent></Card>)}</div>}</section>

      <section className="space-y-3"><h2 className="text-lg font-semibold">Schedule history</h2><div className="overflow-x-auto border-y border-border"><table className="w-full min-w-[720px] text-sm"><thead className="bg-muted/40 text-left text-muted-foreground"><tr><th className="px-3 py-2">Schedule</th><th className="px-3 py-2">Parent</th><th className="px-3 py-2">Due</th><th className="px-3 py-2">Amount</th><th className="px-3 py-2">Received</th><th className="px-3 py-2">Written off</th><th className="px-3 py-2">State</th></tr></thead><tbody>{item.schedules.map((schedule) => <tr key={schedule.id} className="border-t border-border"><td className="px-3 py-2">#{schedule.id}</td><td className="px-3 py-2">{schedule.parent_id ? `#${schedule.parent_id}` : "Original"}</td><td className="px-3 py-2">{formatDisplayDate(schedule.due_date)}</td><td className="px-3 py-2">{schedule.amount}</td><td className="px-3 py-2">{schedule.received_amount}</td><td className="px-3 py-2">{schedule.written_off_amount}</td><td className="px-3 py-2"><Badge variant="outline">{READ_STATE_LABELS[schedule.read_state] || schedule.read_state}</Badge></td></tr>)}</tbody></table></div></section>
